...
- 普通开发从 `master` 创建 `feature/*`、`fix/*` 或 `docs/*` 分支，PR 默认目标是 `master`。`master` 是日常集成主分支，通过 CI 即可合并，无需同时发布版本。
- `release` 是长期发布分支；正常发布通过 `master → release` PR，使用 merge commit。合入 `release` 才触发正式发布；不保留长期 `staging` 分支。
...
> 请按 `AGENTS.md` 和 `docs/branching-and-releases.md` 处理此任务。先检查工作区与分支，从 master 创建开发分支，完成实现和必要测试，准备合入 master 的 PR。若任务是发布，先核对 SemVer、versionCode、公开更新源与签名配置，准备 master 到 release 的发布 PR；合并后使用 GitHub CLI 跟踪 release workflow，并验证 Release APK 与 SHA-256。遵循本次用户已明确的提交、推送、合并和发布授权范围。
