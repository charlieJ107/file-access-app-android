...
- **SMB 上传暂不支持按字节断点续传。** 暂停或系统中断后，同一任务会核对回执，未提交的内容可能从头重传。自动备份每个运行时段最多约7分钟，单文件超过256 MiB或大小未知时等待用户点击“立即备份”启动较长的 UIDT。完整的大视频自动备份仍需后续断点上传能力。
...
- 内部上传回执 `.fileaccess-upload-*.receipt` 约1 KiB/文件；提交成功后保留，供未知结果恢复使用，并在 App 列表隐藏。显式删除空目录只清理已确认提交的内部回执；未知/损坏/未完成的回执保留并报告冲突。历史回执和取消任务的远端临时文件尚无自动清理功能。
...
- 普通上传确认 SMB 写入应答及远端长度；完整远端 SHA-256 回读用于未知结果恢复。服务端存储故障和绕过 SMB 直接改盘仍需真实 NAS 故障验证。
...
先在用户 Android16 手机与 SMB NAS 上验证目录、预览、后台传输和授权恢复，再优先补充 SMB 大文件断点上传、远端临时文件安全清理、后台扫描与断电恢复矩阵。完成这些可靠性验收后，再按统一能力契约扩展其他协议与私有网盘。
